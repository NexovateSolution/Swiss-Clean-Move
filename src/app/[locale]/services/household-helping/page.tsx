...
export default function HouseholdHelpingPage({ params: { locale } }: { params: { locale: string } }) {
    const t = useTranslations();

    const features = (t.raw('services.features.householdHelping') as string[]) || [];
    ...
    return (
        <Layout>
            {/* Hero Section */}
            <SwissHero
                badge={t('home.services.householdHelping.title')}
                title={t('home.services.householdHelping.title')}
                subtitle={t('home.services.householdHelping.description')}
                cta={
                ...
                right={
                    <div className="w-full h-[340px] md:h-[420px] bg-swiss-section flex items-center justify-center">
                        <div className="w-20 h-20 bg-swiss-softRed rounded-3xl flex items-center justify-center mx-auto border border-swiss-border shadow-subtle">
                            <Heart className="w-10 h-10 text-swiss-red" />
                        </div>
                    </div>
                }
            />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        <div className="space-y-6">
                            <h2 className="text-3xl font-bold text-swiss-gray-800">
                                {t('home.services.householdHelping.title')}
                            </h2>
                            <p className="text-lg text-swiss-gray-700">
                                {t('home.services.householdHelping.description')}
                            </p>
                            ...
                            <div className="bg-swiss-blue/10 p-6 rounded-lg">
                                <div className="text-2xl font-bold text-swiss-blue mb-2">{t('services.pricing.householdHelping')}</div>
                            </div>
                        </div>
                        <div className="relative">
                            <div className="bg-swiss-gray-200 rounded-2xl h-96 flex items-center justify-center">
                                <div className="text-center space-y-4">
                                    <Heart className="w-16 h-16 text-swiss-blue mx-auto" />
                                    <p className="text-swiss-gray-600">{t('home.services.householdHelping.title')}</p>
                                </div>
                            </div>
                        </div>
                    </div>
