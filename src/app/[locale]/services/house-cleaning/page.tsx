...
export default function HouseCleaningPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const features = (t.raw('services.features.houseCleaning') as string[]) || [];
  ...
  return (
    <Layout>
      {/* Hero Section */}
      <SwissHero
        badge={t('home.services.houseCleaning.title')}
        title={t('home.services.houseCleaning.title')}
        subtitle={t('home.services.houseCleaning.description')}
        cta={
          <div className="flex flex-col sm:flex-row gap-4 justify-start">
            <Link href={`/${locale}/form?service=house-cleaning`} className="btn-secondary text-lg px-8 py-4">
        ...
        right={
          <div className="w-full h-[340px] md:h-[420px] bg-swiss-section flex items-center justify-center">
            <div className="w-20 h-20 bg-swiss-softRed rounded-3xl flex items-center justify-center mx-auto border border-swiss-border shadow-subtle">
              <HomeIcon className="w-10 h-10 text-swiss-red" />
            </div>
          </div>
        }
      />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              ...
              <p className="text-lg text-swiss-gray-700">
                {t('home.services.houseCleaning.description')}
              </p>
              ...
              <div className="bg-swiss-blue/10 p-6 rounded-lg">
                <div className="text-2xl font-bold text-swiss-blue mb-2">{t('services.pricing.houseCleaning')}</div>
              </div>
            </div>
            <div className="relative">
              <div className="bg-swiss-gray-200 rounded-2xl h-96 flex items-center justify-center">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-swiss-gray-600">{t('home.services.houseCleaning.title')}</p>
                </div>
              </div>
            </div>
          </div>
