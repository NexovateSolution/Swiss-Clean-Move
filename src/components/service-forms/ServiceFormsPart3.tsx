...
export function SpecialCleaningForm({ step, d, set, tl, v, arrHas, toggleArr, ImageUpload }: FormStepProps) {
  switch (step) {
    case 0:
      return (
        <div>
          <SH>{tl('wizard.specialCleaning.project')}</SH>
          <FI label={tl('wizard.specialCleaning.address')} value={v('address')} onChange={v => set('address', v)} required />
          <FS label={tl('wizard.specialCleaning.propertyType')} value={v('propertyType')} onChange={v => set('propertyType', v)}
            options={[
              { value: 'private', label: tl('wizard.specialCleaning.types.private') },
              { value: 'commercial', label: tl('wizard.specialCleaning.types.commercial') },
              { value: 'industrial', label: tl('wizard.specialCleaning.types.industrial') },
              { value: 'public', label: tl('wizard.specialCleaning.types.public') }
            ]}
          />
          <div className="grid grid-cols-2 gap-4">
            <FI label={tl('wizard.specialCleaning.area')} value={v('area')} onChange={v => set('area', v)} type="number" required />
          </div>

          <SH>{tl('wizard.specialCleaning.typeOfCleaning')}</SH>
          {[
            { k: 'facade', l: tl('wizard.specialCleaning.typesCleaning.facade') },
            { k: 'carpet', l: tl('wizard.specialCleaning.typesCleaning.carpet') },
            { k: 'escalator', l: tl('wizard.specialCleaning.typesCleaning.escalator') },
            { k: 'industrial', l: tl('wizard.specialCleaning.typesCleaning.industrial') },
            { k: 'solar', l: tl('wizard.specialCleaning.typesCleaning.solar') },
            { k: 'graffiti', l: tl('wizard.specialCleaning.typesCleaning.graffiti') },
            { k: 'event', l: tl('wizard.specialCleaning.typesCleaning.event') }
          ].map(s => ( <FC key={s.k} label={s.l} checked={arrHas('cleaningTypes', s.k)} onChange={() => toggleArr('cleaningTypes', s.k)} /> ))}
        </div>
      );
    case 1:
      return (
        <div>
          <SH>{tl('wizard.specialCleaning.equipment')}</SH>
          <FC label={tl('wizard.specialCleaning.equipmentNeeds.scaffolding')} checked={arrHas('equipmentNeeds', 'scaffolding')} onChange={() => toggleArr('equipmentNeeds', 'scaffolding')} />
          <FC label={tl('wizard.specialCleaning.equipmentNeeds.lifting')} checked={arrHas('equipmentNeeds', 'lifting')} onChange={() => toggleArr('equipmentNeeds', 'lifting')} />
          <FC label={tl('wizard.specialCleaning.equipmentNeeds.waterConnection')} checked={arrHas('equipmentNeeds', 'waterConnection')} onChange={() => toggleArr('equipmentNeeds', 'waterConnection')} />
          <FC label={tl('wizard.specialCleaning.equipmentNeeds.powerConnection')} checked={arrHas('equipmentNeeds', 'powerConnection')} onChange={() => toggleArr('equipmentNeeds', 'powerConnection')} />

          <SH>{tl('wizard.specialCleaning.contamination')}</SH>
          <FS label={tl('wizard.specialCleaning.contaminationLevel')} value={v('contaminationLevel')} onChange={v => set('contaminationLevel', v)}
            options={[
              { value: 'light', label: tl('wizard.specialCleaning.levels.light') },
              { value: 'medium', label: tl('wizard.specialCleaning.levels.medium') },
              { value: 'heavy', label: tl('wizard.specialCleaning.levels.heavy') },
              { value: 'extreme', label: tl('wizard.specialCleaning.levels.extreme') }
            ]}
          />
          <ImageUpload />
        </div>
    ...
    case 2:
      return (
        <div>
           <SH>{tl('wizard.specialCleaning.schedule')}</SH>
          <FI label={tl('wizard.specialCleaning.desiredDate')} value={v('desiredDate')} onChange={v => set('desiredDate', v)} type="date" required />
          <FTA label={tl('wizard.specialCleaning.notes')} value={v('notes')} onChange={v => set('notes', v)} />

          <SH>{tl('wizard.specialCleaning.contact')}</SH>
          <FI label={tl('wizard.specialCleaning.company')} value={v('company')} onChange={v => set('company', v)} />
          <FI label={tl('wizard.specialCleaning.nameContact')} value={v('nameFirstName')} onChange={v => set('nameFirstName', v)} required />
          <FI label={tl('wizard.specialCleaning.email')} value={v('emailAddress')} onChange={v => set('emailAddress', v)} type="email" required />
          <FI label={tl('wizard.specialCleaning.phone')} value={v('telephone')} onChange={v => set('telephone', v)} type="tel" required />
        </div>
      );
    default: return null;
  }
